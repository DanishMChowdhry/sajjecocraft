                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="vendor" id="vendor" class="form-control @error('vendor') is-invalid @enderror">
                                    ...
                                    @foreach ($vendor as $item)
                                        <option value="{{ $item->id }}" {{ old('vendor') == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div id="vendor-charges-container" style="display: none;" class="mb-3 border p-3 rounded bg-light">
                                ...
                                @foreach ($fields as $field)
                                    <div class="mb-2">
                                        <label class="form-label text-capitalize">{{ ucwords(str_replace('_', ' ', $field)) }}</label>
                                        <input type="text" id="{{ $field }}" class="form-control vendor-charge" readonly>
                                    </div>
                                @endforeach

                                <div class="mt-3">
                                    <label class="form-label fw-bold">Total Vendor Charges</label>
                                    <input type="text" name="total_vendor_charges" id="total_vendor_charges" class="form-control" readonly>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="purchase_price" id="purchase_price" class="form-control @error('purchase_price') is-invalid @enderror" value="{{ old('purchase_price') }}">
                                @error('purchase_price') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="mb-3">
                                <label for="selling_price" class="form-label">Selling Price</label>
                                <input type="text" name="selling_price" id="selling_price" class="form-control @error('selling_price') is-invalid @enderror" value="{{ old('selling_price') }}">
                                @error('selling_price') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="mb-3">
                                <label for="discounted_rates" class="form-label">Discounted Price</label>
                                <input type="text" name="discounted_rates" id="discounted_rates" class="form-control @error('discounted_rates') is-invalid @enderror" value="{{ old('discounted_rates') }}">
                                @error('discounted_rates') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
